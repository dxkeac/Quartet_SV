...
helpdoc(){
    cat <<EOF
Description:
    This shell script is used to compare query SV set with benchmark SV set or comparison within quartet,
    and only allow variants with same types to be merged regardless of strand.

    Prerequisites: Jasmine--a tool to merge structural variants (SVs) across samples. The path of Jasmine 
    (e.g.,/public/home/fan_lab/xkduan/software/Jasmine-1.0.1) should be in the environment variable \$PATH_JASMINE.
    In addition, Bcftools (plug-ins mendelian) is required when performing Mendelian analysis.

Usage:
SV discovery:
    $0 -p <B/Bench> -i <VCF list files>
SV genotyping:
    $0 -p <B/Bench> -i <VCF list files> -g <G2> or
    $0 -p <B/Bench> -i <VCF list files> -g <T/True>
SV reproducibility:
    $0 -p <Q/Quartet> -i <VCF list files>
SV Mendelian concordance rate:
    $0 -p <Q/Quartet> -i <VCF list files> -g <T/True>

Option:
    -p    The performance evaluation (B/Bench:use benchmark set comparison; Q/Quartet:use quartet comparison) (Required)
    -i    VCF list files in order: B/Bench: benchmark set and query set; Q/Quartet: D5, D6, F7 and M8 (Required)
    -t    SV type to compare (choose one from INS,DEL,DUP,INV and BND) [default: all] (Optional)
    -g    Consider the genotype of the consensus variants when comparing (choose one from T/True and G2, 
          T/True: open; G2: INS and DEL are considered due to the current benchmark set only supporting their genotypes)  
...
while getopts "h:p:i:t:g:e:m:s:k:" opt
do
    case $opt in
        h)
            helpdoc
...
echo -e "\nThe precision and recall of SV genotyping:\nINS and DEL type"
cat ${vcf_list%.*}.vcf|grep -E "SVTYPE=INS|SVTYPE=DEL"| \
awk -v OFS="\t" \
'BEGIN{Benchmark=0;Query=0;shared=0;geno=0}{match($0,/SUPP_VEC=([0-9]+)/,a);split(a[1],vec,"");Benchmark+=vec[1];Query+=vec[2];if(vec[1]+vec[2]==2){num=2+9;if($num !~ /\.\/\./){shared+=1}{split($10,truth,":");split($num,sample,":")}{if(truth[1]==sample[1]){geno+=1}}}}END{print "Bench_num","Query_num","TP_num","Precision","Recall","F1_score","\n",Benchmark,Query,geno,substr(geno/Query,1,6),substr(geno/Benchmark,1,6),substr(2*(geno/Benchmark)*(geno/Query)/((geno/Benchmark)+(geno/Query)),1,6)}'
...
then
    ...
    #header=/public/home/fan_lab/xkduan/Quartet_data/SV_combine/pacbio_ont_pacbio2_combine_3/quartet_geno/vcf_header.txt
    #header=`grep "#" ${vcf_list%.*}.vcf|sed 's/\tINFO.*/\tINFO\tFORMAT\tmother\tfather\tchild/g'`
    input=${vcf_list%.*}.vcf
    ...
    Quartet_union=`grep -v "#" $input|grep -v -E "chrX|chrY|chrM"|wc -l`
    ...
    input=${vcf_list%.*}.vcf
    Mendelian=`bcftools +mendelian <(cat <(grep "#" ${vcf_list%.*}.vcf|sed 's/\tINFO.*/\tINFO\tFORMAT\tmother\tfather\tchild/g') <(grep -E "SUPP_VEC=1111" $input|grep -E -v "\.\/\."|grep -E -v "0\/0.*0\/0.*0\/0.*0\/0.*"| \
    grep -v -E "chrX|chrY|chrM"|grep -E SVTYPE=$SV|\
    awk -v OFS="\t" '/#/{print}{split($10,LCL5,":");split($11,LCL6,":")}{if(LCL5[1]==LCL6[1]){print $0}}' -| \
    perl -lane 'if(/SVTYPE=(.*?);.*?GT.*?(.\/.):.*?(.\/.):.*?(.\/.):.*?(.\/.):.*?/){$type="SVTYPE=".$1;$m=$5;$f=$4;$c=$2;print join("\t",@F[0..5],"PASS",$type,"GT",$m,$f,$c)}')) \
    -t mother,father,child -l+|grep -v "#"|wc -l`
    Quartet_union=`grep -v "#" $input|grep -v -E "chrX|chrY|chrM"|grep -E SVTYPE=$SV|wc -l`
